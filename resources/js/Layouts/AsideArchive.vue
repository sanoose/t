...
onMounted  (() => { 
        ...
        if ( localStorage.getItem("show_settings") ) {
          document.querySelector(".show_settings").click() ;
        } 
    }) ;
    ...
    function ShowSettings ()  {
          let  show_settings =  document.querySelector(".show_settings")  , 
          settings  =  document.querySelector(".settings")  ; 
          show_settings.classList.toggle("settings_active") ; 
          if ( show_settings.classList.contains("settings_active") ) {
...
<template>
   <aside class="flex-shrink-0 p-3  aside"  >
    ...
    <div  class="d-flex align-items-center flex-column  pb-3 mb-3  text-decoration-none border-bottom">
      <h6 class="fs-5 fw-semibold"> منظومة الصادر والوارد    </h6>
    ...
    <ul class="list-unstyled ps-0">
      ...
      <li class="mb-1">
        <a class="btn  align-items-center rounded " >
            <Link  :href="route('Archiving.HomeArchive')" class="rounded"
              :class = "{'active_link' :  route().current('Archiving.HomeArchive')  }" >   <fa  icon = "home" /> الرئيسية </Link> 
        </a>
      </li>

      <li class="mb-1  dropdown_list">
     
     <button class="btn btn-toggle align-items-center rounded collapsed" data-index = "0"  
       data-bs-toggle="collapse" data-bs-target="#dashboard-collapse0"  aria-expanded= "false"  
      >
        <fa  icon = "paper-plane" /> ادارة   البريد الصادر <fa  :icon = "icon[0]" class="arrow"  />
     </button>
     <div class="collapse" id="dashboard-collapse0" >
      ...
      <hr>
  <li class="mb-1 show_settings"  @click="ShowSettings" >
        <a class="btn  align-items-center rounded " >
          <span class="rounded ">   <fa  icon = "gear" class="gear" />  
                الإعدادات
             <!-- الإعدادات   -->
             </span> 
        </a>
     ...
     <button class="btn btn-toggle align-items-center rounded collapsed" data-index = "1"  
       data-bs-toggle="collapse" data-bs-target="#dashboard-collapse1"  aria-expanded= "false"  
      >
        <fa  icon = "key" /> ادارة نوع البريد <fa  :icon = "icon[1]" class="arrow"  />
     </button>

     <div class="collapse" id="dashboard-collapse1" >
       <ul class="btn-toggle-nav list-unstyled fw-normal pb-1 small nested_ul">
            <li>   
              <Link  :href="route('Archiving.Settings.LetterTypes.create')" class="rounded"
                        :class = "{'active_link' :  route().current('Archiving.Settings.LetterTypes.create') }" >
                        <fa  icon = "square-plus" /> 
                        اضافة نوع بريد
            </Link> 
            </li>
            <li>   
              <Link  :href="route('Archiving.Settings.LetterTypes.index')" class="rounded"
              :class = "{'active_link' : route().current('Archiving.Settings.LetterTypes.index')}">
              <fa  icon = "eye" /> 
              عرض أنواع البريد
              </Link> 
            </li>
            ...
            <li>   
              <Link  :href="route('Archiving.Settings.LetterTypes.Deleted')" class="rounded"
              :class = "{'active_link' : route().current('Archiving.Settings.LetterTypes.Deleted') }" >
              <fa  icon = "trash" /> 
                أنواع البريد  المحذوفة
              </Link> 
            </li>
        ...
        <button class="btn btn-toggle align-items-center rounded collapsed" data-index = "2"  
          data-bs-toggle="collapse" data-bs-target="#dashboard-collapse2"  aria-expanded= "false"  
          >
            <fa  icon = "book" /> ادارة   البنود <fa  :icon = "icon[2]" class="arrow"  />
        </button>

        <div class="collapse" id="dashboard-collapse2" >
          <ul class="btn-toggle-nav list-unstyled fw-normal pb-1 small nested_ul">
                <li>   
                  <Link  :href="route('Archiving.Settings.Subjects.create')" class="rounded"
                            :class = "{'active_link' :  route().current('Archiving.Settings.Subjects.create') }" >
                            <fa  icon = "square-plus" /> 
                            اضافة   بند جديد
                </Link> 
                </li>
                <li>   
                  <Link  :href="route('Archiving.Settings.Subjects.index')" class="rounded"
                  :class = "{'active_link' : route().current('Archiving.Settings.Subjects.index')}">
                  <fa  icon = "eye" /> 
                  عرض   البنود
                  </Link> 
                </li>
                ...
                <!-- <li>   
                  <Link  :href="route('Archiving.Settings.Subjects.Deleted')" class="rounded"
                  :class = "{'active_link' : route().current('Archiving.Settings.Subjects.Deleted') }" >
                  <fa  icon = "trash" /> 
                      البنود    المحذوفة
                  </Link> 
                </li> -->
