...
function ReferenceNumberSearch(e) {
  form.get(route("Archiving.ReferenceNumbers.ReferenceNumbersDeleted"), { preserveState: true });
}

function  Restore(  id , name ) {
    Swal.fire({
        title: "استرجاع جهة البلاغ",
        icon: 'warning',
        text: ` هل انت متأكد من استرجاع   الإشاري       ${name}  ؟`,
        type: "warning",
        ...
        preConfirm: () => {
            ...
            router.visit(route('Archiving.ReferenceNumbers.ReferenceNumbersRestore',id), 
            { method: 'get'  ,  preserveScroll: true  ,
            onSuccess : () =>  toast.success("تم الإسترجاع بنجاح") ,
            onError   : () =>  toast.error("خطأ لم يتم الإسترجاع ")   ,
...
<Head title="عرض الأرقام الإشارية المحذوفة " />

        <AppLayoutArchive>
                    <h3> عرض   الأرقام الإشارية المحذوفة   </h3>
                    <hr>
                    <div class="row">
                    <div class="col-md-12">
                                            ...
                                            <td>
                                                ...
                                                <ul class="dropdown-menu">
                                                    ...
                                                    <li><span class="dropdown-item btn_edit btn" 
                                                            @click="Restore(reference_number.id , 
                                                            `
